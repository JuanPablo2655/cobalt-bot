    ...
    try {
        ...
        const govtInfo = async nation => {
            const request = await fetch(
                `https://www.nationstates.net/cgi-bin/api.cgi?nation=${nation}&q=govt+name+flag+fullname`,
            );
        ...
        if (!args[0]) {
            nations.findOne(
                ...
                async (err, res) => {
                    if (err) cb(err);
                    if (!res) {
                        message.channel.send({ content: 'Please connect nationstates or define the nation' });
                    } else {
                        var nationName = res.nation.toLowerCase();
                        return message.channel.send({ embeds: [await govtInfo(nationName)] });
                    }
                },
            );
        } else {
