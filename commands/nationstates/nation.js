    ...
    try {
        ...
        const nationInfo = async nation => {
            const request = await fetch(
                `https://www.nationstates.net/cgi-bin/api.cgi?nation=${nation}&q=name+gdp+population+category+currency+region+wa+flag+fullname+motto+influence+founded+lastactivity+demonym`,
            );
            ...
            nations.findOne(
                ...
                async (err, res) => {
                    ...
                    if (!res) {
                        ...
                        return message.channel.send({ embeds: [await nationInfo(nationName)] });
                    }
                },
            );
        } else {
