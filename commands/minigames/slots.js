...
module.exports.run = async (cobalt, message, args, addCD, cb) => {
    try {
        ...
        let money = args[0];
        if (!money || (isNaN(money) && money !== 'all' && money !== 'max'))
            return message.channel.send({ content: 'Please enter an amount to play slots' });
        if (money < 50) return message.channel.send({ content: `The minimum you can gamble is \`₡100\`.` });
        if (money == 'all' || money == 'max') money = userData.onHand;
        else money = parseInt(args[0]);
        if (0 >= money) return message.channel.send({ content: 'Please enter a positive number and more than ₡0' });
        if (money > userData.onHand) return message.channel.send({ content: 'You dont have enough CND' });
        ...
        if (first_2 && last_2) {
            ...
            let moneyEarned = Math.floor(money * 0.5);
            userData.onHand += moneyEarned;
            userData.netWorth += moneyEarned;
            await userData.save();
        ...
        message.channel.send({ content: '|-' + chosen.join('-') + '-|\n`' + msg + '`' });
    } catch (e) {
        cb(e);
    }
};
