...
let arrayOfWeights = [56, 21, 10, 13];
...
module.exports.run = async (cobalt, message, args, cb) => {
    try {
        ...
        if (robbedUser.onHand == 0) {
            return message.channel.send(`${member.user.username} doesn\'t have cobaltion money`)
        } else {
            if (random === 0) {
                ...
                message.channel.send(`${member.user.username} fought back, you left without a single cobaltion dollar.`)
            } else if (random === 2) {
                user.onHand = 0
                user.deposited = 0
                user.bankSpace = 1000
